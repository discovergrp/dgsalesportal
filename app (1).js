...
function filteredLeads() {
  ...
  return allLeadsCache.filter(l => inRange(leadDate(l), range));
}
...
function renderLeadsTable() {
  ...
  // Admins and sales admins see the whole team, so the heading shouldn't
  // claim otherwise.
  const seesEveryone = currentProfile && currentProfile.role !== "agent";
  const sub = box.querySelector(".card-title-row p");
  if (sub) sub.textContent = seesEveryone ? "All consultants' records" : "Personal records only";
  const owner = document.getElementById("leadsOwner");
  if (owner && seesEveryone) owner.textContent = "Leads Tracker — All Consultants";
    ...
    <table style="width:100%; border-collapse:collapse; min-width:900px;">
      <thead>
        <tr>
          ...
          <th style="${th}">Date inquired</th>
          <th style="${th}">Travel date</th>
          <th style="${th} text-align:center;">Pax</th>
          <th style="${th}">Email address</th>
          <th style="${th}">Contact number</th>
          ${seesEveryone ? `<th style="${th}">Consultant</th>` : ""}
      ...
      <tbody>
        ${leads.map(l => `
          <tr data-lead="${l.id}">
            ...
            <td style="${td}">${l.client_email || "—"}</td>
            <td style="${td}">${l.client_mobile || "—"}</td>
            ${seesEveryone ? `<td style="${td}">${agentName(l.agent_id)}</td>` : ""}
            <td style="${td}">
            ...
            <td style="${td} white-space:nowrap;">
              <a href="#" class="lead-open" data-lead="${l.id}" style="font-size:13px; font-weight:700; color:var(--gold-600); text-decoration:none;">View profile →</a>
            </td>
          </tr>`).join("")}
      </tbody>
    </table>`;

  // "View profile" opens the client's full record over in Client's Documents.
  wrap.querySelectorAll(".lead-open").forEach(link => {
    link.addEventListener("click", (e) => {
      e.preventDefault();
      const sel = document.getElementById("voucherClientSelect");
      if (!sel) return;
      sel.value = link.dataset.lead;
